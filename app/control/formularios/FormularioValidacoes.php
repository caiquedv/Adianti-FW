...
class FormularioValidacoes extends TPage
{
    ...
    public function __construct()
    {
        ...
        $this->form->setClientValidation(true); // ativa a validação do html (impede a req)
                                                // sem isso os erros retornam do servidor
                                                // opcional pois quando a campos nao visiveis pode atrapalhar o user
        ...
        $field1->addValidation('Field 1', new TMinLengthValidator, [3]); // comprimento minimo
        $field1->addValidation('Field 2', new TRequiredValidator); // campo obrigatório
        $field2->addValidation('Field 2', new TMaxLengthValidator, [20]); // comp max
        $field3->addValidation('Field 3', new TMinValueValidator, [1]); // valor numerico minimo
        $field4->addValidation('Field 4', new TMaxValueValidator, [10]); // valor numerico maximo
        $field5->addValidation('Field 5', new TRequiredValidator);
        $field6->addValidation('Field 6', new TRequiredValidator); 
        $field6->addValidation('Field 6', new TEmailValidator); // email
        $field7->addValidation('Field 7', new TNumericValidator); // somente numeros
    ...
    public function onSend($param)
    {
        try { //
            ...
            $this->form->validate(); // valida o formulario (usa Exception)
